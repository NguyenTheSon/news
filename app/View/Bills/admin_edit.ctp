  ...
  <div class="panel-body">
	    <div class="row">
			...
			<div class="col-md-12 userinfo" style="">
				<input type="hidden" name="data[Bill][user_id]" id="user_id" value="<?php echo $this->request->data['Bill']['user_id'];?>">
				<table class=' col-md-6 table table-striped table-bordered table-hover table-primary table-hover'>
				<tr>
					<td>Họ Tên</td>
					<td class="name"><?php echo $this->request->data['User']['name'];?></td>
				</tr>
				<tr>
					<td>Số điện thoại</td>
					<td class="phonenumber"><?php echo $this->request->data['User']['phonenumber'];?></td>
				</tr>
				<tr>
					<td>Địa chỉ</td>
					<td class="address"><?php echo $this->request->data['User']['address'];?></td>
				</tr>
				<tr>
					<td>Số dư</td>
					<td class="balance"><?php echo number_format($this->request->data['User']['balance']);?></td>
				</tr>
				</table>
			</div>
			<?php echo $this->Form->input('discount', array('class' => 'form-control form-group', 'label' => 'Khuyến Mại')); ?>
			<?php echo $this->Form->input('pay_type', array('class' => 'form-control form-group', 'label' => 'Hình thức thanh toán','options' => array("0" => "Tiền mặt", "1" => "Tài khoản thẻ"),'default' => $this->request->data['Bill']['pay_type'])); ?>
				...
				<fieldset class="list-category-border">
	            	...
	            	<table class="money_detail">
	            		<tr><td><b>Tổng tiền:</b></td><td><span class="total_money"><?php echo number_format($this->request->data['Bill']['total_money']);?></span></td></tr>
	            		<tr><td><b>Khuyến mại:</b></td><td><span class="total_discount"><?php echo number_format($this->request->data['Bill']['discount']);?></span></td></tr>
	            		<tr><td><b>Thành tiền:</b></td><td><span class="money"><?php echo number_format($this->request->data['Bill']['total_money'] - $this->request->data['Bill']['discount']);?></span></td></tr>
	            	</table>
			...
			<?php echo $this->Form->button('Sửa Hóa Đơn', array('class' => 'btn btn-primary')); ?>
			<?php echo $this->Html->link('Xóa', array('action' => 'delete',$this->data['Bill']['id']), array('class' => 'btn btn-danger pull-right'),__("Bạn có chắc chắn muốn xóa hóa đơn?")); ?>
...
function calculatemoney(){
	...
	var discount = parseInt($(".total_discount").html().replace(/,/g,''));
