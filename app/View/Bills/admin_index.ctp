  ...
  <div class="panel-body">
	    <div class="row">
            <div class="col-sm-3 col-md-2 col-xs-2 search-form pull-left">
                <?php echo $this->Html->link('Tạo hóa đơn', array('action' => 'admin_add', 'admin' => true), array('class' => 'btn btn-primary')); ?>
            </div>
            <div class="col-sm-2 col-md-2 col-xs-2 search-form pull-left">
                ...
                echo $this->Form->select('filter_by_date',
                    ...
                    array(
                        'default' => $this->Session->read('Bills.filter'),
                        'class' => 'form-control',
                        'id' => 'FilterByDate',
                        'empty' => false,
                        'onchange' => 'customize()'
            ...
            <div class="col-sm-4 col-md-4 col-xs-4 search-form pull-left">
                ...
                <div class="row" id="hidden-input" style="display:none;">
                    <div class="input-group">
                        <input class="form-control" type="date" onchange="filter_submit()" id="begin_date" value="<?php echo $this->Session->read('Bills.begin_date') ?>">
                        <span class="input-group-addon">To</span>
                        <input class="form-control" type="date" onchange="filter_submit()" id="end_date" value="<?php echo $this->Session->read('Bills.end_date') ?>">
                    </div>
                </div>
            </div>

            <div class="col-sm-3 col-md-3 col-xs-5 search-form pull-right">
                <?php echo $this->Form->create('Bills', array('action'=>'index','id'=>'searchFormProduct'));?>
                    <div class="input-group">
                        <?php echo $this->Form->input('keyword', array('label' => '', 'class' => 'form-control input-sm', 'placeholder' => 'Tìm kiếm', 'value' => $this->Session->read('Bills.filter_keyword'))); ?>
        ...
        <div class="row">
        	<div class="col-sm-12 col-md-12" id="ListProducts">
            	<?php echo $this->element('Bills/admin_index'); ?>
            </div>
		</div>
    ...
    function customize() {
        if($("#FilterByDate").val() == 5) {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    'data[Bills][filter]': $("#FilterByDate").val(),
                    'data[Bills][begin_date]': begin_date.format('Y-m-d'),
                    'data[Bills][end_date]': end_date.format('Y-m-d')
                },
...
<?php
  ...
  $this->Js->get('#searchFormProduct')->event(
    'submit',
    $this->Js->request(
      array('action' => 'index', 'controller' => 'Bills',),
      array(
        'update' => '#resultField',
        'data' => $data,
        'async' => true,    
